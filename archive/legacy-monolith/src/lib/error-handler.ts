...
export interface AppError {
  ...
  code?: string;
  statusCode?: number;
  action?: 'login' | 'contact_admin' | 'retry' | 'upgrade_role';
...
const ERROR_MESSAGES: Record<ErrorType, { title: string; message: string; action?: string }> = {
  ...
  ROLE_REQUIRED: {
    ...
    message: 'Your role doesn\'t have access to this feature. Please contact your administrator.',
    action: 'contact_admin'
  },
  NOT_FOUND: {
    title: 'Not Found',
...
class ErrorHandler {
  ...
  static parseApiError(error: any, statusCode?: number): AppError {
    ...
    if (statusCode === 401) {
      ...
      type = 'ENCODING_ERROR';
    } else if (errorMessage.toLowerCase().includes('admin access required')) {
   ...
   */
  static permissionDenied(requiredPermission?: Permission, requiredRole?: string): AppError {
    return {
      type: 'PERMISSION_DENIED',
