...
class EmailService {
  ...
  async sendEmail(payload: EmailPayload): Promise<boolean> {
    try {
      // 1. Fetch system setting for email routing rules via raw query as they live in admin schema
      const routingSettings: any[] = await prisma.$queryRaw`SELECT value FROM "SystemSetting" WHERE key = 'emailRoutingRules' LIMIT 1`;
      const routingSettingStr = routingSettings[0];
      ...
      // 2. Fetch all active email providers from Integration table via raw query
      let activeProviders: any[] = await prisma.$queryRaw`SELECT * FROM "Integration" WHERE type = 'email_provider' AND "isActive" = true`;
      ...
      if (assignedProviderName) {
        const primary = activeProviders.find((p: any) => p.name === assignedProviderName);
        if (primary) {
      ...
      const others = activeProviders.filter((p: any) => p.name !== assignedProviderName);
