...
function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navigationItems: NavigationItem[] = [
    ...
    },
    {
      ...
      icon: '📝',
      children: [
        { label: 'All Articles', href: '/admin/content', icon: '📄' },
        { label: 'Web Stories', href: '/admin/content/web-stories', icon: '📱' },
        { label: 'Categories', href: '/admin/content/categories', icon: '🏷️' },
        { label: 'Tags', href: '/admin/content/tags', icon: '🔖' },
        { label: 'Drafts', href: '/admin/content/drafts', icon: '✏️' }
      ]
    },
    {
      ...
      children: [
        ...
        { label: 'Subscribers', href: '/admin/users/subscribers', icon: '📧' },
        { label: 'Permissions', href: '/admin/users/permissions', icon: '🔐' }
      ]
    },
    {
      ...
      children: [
        ...
        { label: 'Traffic', href: '/admin/analytics/traffic', icon: '🌐' },
        { label: 'Content Performance', href: '/admin/analytics/content', icon: '📊' },
        { label: 'User Engagement', href: '/admin/analytics/engagement', icon: '🎯' }
      ]
    },
    {
      label: 'Advertisement',
      ...
      icon: '💼',
      children: [
        { label: 'Campaigns', href: '/admin/advertising', icon: '📢' },
        { label: 'Ad Requests', href: '/admin/advertising/requests', icon: '📋' },
        { label: 'Performance', href: '/admin/advertising/performance', icon: '📈' }
      ]
    },
    {
      label: 'Newsletter',
      ...
      icon: '📧',
      children: [
        { label: 'Campaigns', href: '/admin/newsletter', icon: '📨' },
        { label: 'Templates', href: '/admin/newsletter/templates', icon: '📄' },
        { label: 'Subscribers', href: '/admin/newsletter/subscribers', icon: '👥' }
      ]
    },
    {
      label: 'Media Library',
    ...
    },
    {
      ...
      icon: '💬',
      children: [
        { label: 'Comments', href: '/admin/moderation', icon: '💬' },
        { label: 'Reports', href: '/admin/moderation/reports', icon: '⚠️' },
        { label: 'Spam Filter', href: '/admin/moderation/spam', icon: '🚫' }
      ]
    },
    {
      ...
      icon: '🔒',
      children: [
        { label: 'General', href: '/admin/system', icon: '⚙️' },
        { label: 'Security', href: '/admin/system/security', icon: '🔐' },
        { label: 'Integrations', href: '/admin/system/integrations', icon: '🔗' },
        { label: 'Backup', href: '/admin/system/backup', icon: '💾' }
      ]
    }
  ];
  ...
  const renderNavigationItem = (item: NavigationItem, isChild = false) => {
    ...
    return (
      <div key={item.href} className={isChild ? 'ml-6' : ''}>
        ...
        >
          {/* Animated background */}
          <div className={`absolute inset-0 bg-gradient-to-r from-blue-600/10 to-purple-600/10 transform transition-transform duration-300 ${isActive ? 'translate-x-0' : '-translate-x-full group-hover:translate-x-0'}`}></div>
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark' : ''}`}>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 text-foreground transition-all duration-500 flex">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border/30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl h-[73px]">
              <Link href="/admin" className="flex items-center space-x-3 group">
                ...
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">NewsNerve</h1>
            ...
            {/* User Profile - Matching footer height */}
            <div className="border-t border-border/30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl h-[85px]">
              <div className="flex items-center space-x-3 px-6 py-4 h-full rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 dark:hover:from-blue-900/20 dark:hover:to-purple-900/20 transition-all duration-300 cursor-pointer group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">admin@newsnerve.com</p>
                </div>
        ...
        {/* Main content area - Matching sidebar styling */}
        <div className="flex-1 min-h-screen flex flex-col lg:ml-0 bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
          {/* Admin Top bar - Exact height match with sidebar header */}
          ...
          {/* Page content - Seamlessly connected */}
          ...
          {/* Enhanced Footer - Matching sidebar footer height */}
          <footer className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-t border-border/30 shadow-lg flex-shrink-0 h-[85px]">
            <div className="flex items-center justify-between px-6 py-4 h-full">
              <div className="flex items-center space-x-6 text-slate-600 dark:text-slate-400">
                <div className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full animate-pulse"></div>
                  <p className="font-medium">© 2025 NewsNerve Admin Panel</p>
                </div>
                <span className="hidden md:block text-slate-300 dark:text-slate-600">•</span>
                <p className="hidden md:block font-semibold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                  Version 2.0.0
                </p>
              </div>
              <div className="flex items-center space-x-6">
                <Link 
                  href="/admin/system" 
                  className="flex items-center space-x-2 text-slate-600 dark:text-slate-400 hover:text-green-600 dark:hover:text-green-400 transition-colors duration-300 group"
                >
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                  <span className="group-hover:font-semibold transition-all duration-300">System Online</span>
                </Link>
                <span className="text-slate-300 dark:text-slate-600">•</span>
                <Link 
                  href="/admin/help" 
                  className="text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300 hover:font-semibold"
                >
                  Help Center
                </Link>
                <span className="text-slate-300 dark:text-slate-600">•</span>
                <button className="text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors duration-300 hover:font-semibold group">
                  <span className="flex items-center space-x-2">
                    <span>Logout</span>
                    <span className="group-hover:translate-x-1 transition-transform duration-300">→</span>
                  </span>
