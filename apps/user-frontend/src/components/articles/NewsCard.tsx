    ...
    <Link href={getContentUrl(article)}>
      <div 
        className={`news-card border-l-4 ${
          article.isBreaking ? 'border-vermillion' : 'border-border'
        } bg-card hover:bg-white/50 dark:hover:bg-ash/10 p-4 transition-all duration-200 hover:shadow-lg cursor-pointer group ${className}`}
      >
        {/* Header */}
        <div className="flex items-center gap-2 mb-2">
          ...
          {!article.isBreaking && (
            <span className="bg-ink/5 dark:bg-white/10 text-foreground font-mono text-[10px] tracking-wider uppercase font-semibold px-2 py-1">
              NEWS
            </span>
          )}
        </div>
