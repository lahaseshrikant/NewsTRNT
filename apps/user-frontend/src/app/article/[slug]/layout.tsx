...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      article.summary ||
      `Read "${article.title}" on NewsTRNT — multi-perspective news and analysis.`,
...
export default async function ArticleSlugLayout({
  ...
  const jsonLd = article
    ? {
        ...
        mainEntityOfPage: {
          "@type": "WebPage",
          "@id": `${process.env.NEXT_PUBLIC_SITE_URL || "https://newstrnt.com"}/article/${slug}`,
        },
        articleSection: article.category?.name || "News",
        keywords:
          article.tags?.map((t) => (typeof t === "string" ? t : t.name)).join(", ") || "",
      }
    : null;
