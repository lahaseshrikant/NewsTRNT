...
export function RoleBadge({ 
 ...
 const badge = RBACUtils.getRoleBadge(displayRole as UserRole);
...
export function CurrentUserRole({ 
 ...
 const badge = RBACUtils.getRoleBadge(session.role as UserRole);
 ...
 <span className="font-medium text-sm">{session.fullName || session.username}</span>
 {showEmail && (
 <span className="text-xs text-[rgb(var(--muted-foreground))]">{getEmailString(session.email)}</span>
 )}
 <span className={`text-xs ${badge.color}`}>{badge.displayName}</span>
 </div>
 </div>
 );
}
...
export function PermissionDenied({ 
 ...
 <h2 className="text-2xl font-bold text-[rgb(var(--foreground))] mb-2">{title}</h2>
 <p className="text-[rgb(var(--muted-foreground))] text-center max-w-md mb-4">{message}</p>
 {showRoleInfo && roleInfo && (
 <div className="mt-4 p-4 bg-[rgb(var(--muted))]/10 rounded-lg">
 <p className="text-sm text-[rgb(var(--muted-foreground))] mb-2">Your current role:</p>
 <RoleBadge size="lg" />
 </div>
 )}
 </div>
