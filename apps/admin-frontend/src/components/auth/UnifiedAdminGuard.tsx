...
export default function UnifiedAdminGuard({
 ...
 <p className="text-[rgb(var(--muted-foreground))] mt-4 text-sm">Verifying access...</p>
 </div>
 </div>
 );
 }
 ...
 <h2 className="text-xl font-semibold text-[rgb(var(--foreground))] mb-2">Access Denied</h2>
 <p className="text-[rgb(var(--muted-foreground))] text-sm mb-1">
 {requireSuperAdmin
 ?'This page requires Super Admin privileges.'
 : `Missing permission: ${requiredPermission}`}
 </p>
 <p className="text-[rgb(var(--muted-foreground))] text-xs mb-6">
 Signed in as {user?.email} ({user?.role})
 </p>
 <button
 onClick={() => router.push('/')}
 className="px-4 py-2 bg-[rgb(var(--primary))] text-white rounded-lg hover:opacity-90 text-sm"
 >
 Back to Dashboard
 </button>
 </div>
