...
export function AdminTabs({ tabs, activeTab, onChange, variant = 'underline', size = 'md', className = '' }: AdminTabsProps) {
  ...
  return (
    <div className={`${isUnderline ? 'border-b border-border' : ''} ${className}`}>
      <nav className={`flex ${isUnderline ? 'gap-6 -mb-px' : 'gap-1 bg-secondary/50 p-1 rounded-lg'}`}>
        {tabs.map(tab => {
...
export function AdminModal({ open, onClose, title, description, children, maxWidth = 'md', footer }: AdminModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      />
      {/* Panel */}
      <div className={`relative bg-card rounded-xl border border-border shadow-editorial-lg ${widthMap[maxWidth]} w-full mx-4 animate-fade-in`}>
        {title && (
        ...
        {footer && (
          <div className="px-6 py-4 border-t border-border bg-secondary/30 rounded-b-xl flex items-center justify-end gap-3">
            {footer}
          </div>
        )}
      </div>
...
export function EmptyState({ icon, title, description, action, className = '' }: EmptyStateProps) {
  return (
    <div className={`flex flex-col items-center justify-center py-16 px-4 text-center ${className}`}>
      {icon && <div className="w-16 h-16 rounded-2xl bg-secondary/50 text-muted-foreground flex items-center justify-center mb-4">{icon}</div>}
    ...
    <div className={`space-y-1.5 ${className}`}>
      {(label || showValue) && (
        <div className="flex items-center justify-between text-xs">
          ...
          {showValue && <span className="text-foreground font-semibold">{Math.round(pct)}%</span>}
        </div>
      )}
      <div className={`w-full ${barHeight} bg-secondary rounded-full overflow-hidden`}>
        <div
