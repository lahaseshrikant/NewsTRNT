...
const AdvertisingManager: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'campaigns' | 'proposals' | 'performance' | 'networks'>('campaigns');
          ...
          <div className="border-b border-border">
            <nav className="flex overflow-x-auto space-x-8 px-6">
              {[
                ...
                { id: 'networks', label: 'Ad Networks', icon: '🔌' }
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            {activeTab === 'networks' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h3 className="text-lg font-semibold text-foreground">Ad Network Integrations</h3>
                    <p className="text-sm text-muted-foreground">Connect advertising platforms to monetize your content</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Google AdSense */}
                  <div className="border border-border rounded-2xl p-6 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">Display ads from Google&apos;s advertiser network</p>
                      </div>
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">Not Connected</span>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Publisher ID</label>
                        <input type="text" placeholder="ca-pub-XXXXXXXXXX" className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Ad Slots</label>
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          {['Header Banner', 'Sidebar', 'In-Article', 'Footer'].map(slot => (
                            <label key={slot} className="flex items-center gap-2 p-2 border border-border rounded-lg cursor-pointer hover:bg-muted/40">
                  ...
                  <div className="border border-border rounded-2xl p-6 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">Mobile app monetization with banner, interstitial & rewarded ads</p>
                      </div>
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">Not Connected</span>
                    </div>
                    <div className="space-y-3">
                      <div>
                        ...
                        <input type="text" placeholder="ca-app-pub-XXXXXXXXXX" className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Ad Formats</label>
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          {['Banner', 'Interstitial', 'Rewarded', 'Native'].map(fmt => (
                            <label key={fmt} className="flex items-center gap-2 p-2 border border-border rounded-lg cursor-pointer hover:bg-muted/40">
                              <input type="checkbox" className="rounded" />
                              <span>{fmt}</span>
                            </label>
                          ))}
                  ...
                  <div className="border border-border rounded-2xl p-6 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center text-2xl">📘</div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-foreground">Meta Audience Network</h4>
                        <p className="text-sm text-muted-foreground">Display ads from Meta&apos;s advertising platform</p>
                      </div>
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">Not Connected</span>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Placement ID</label>
                        <input type="text" placeholder="YOUR_PLACEMENT_ID" className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" />
                      </div>
                  ...
                  {/* Custom Ad Provider */}
                  <div className="border border-dashed border-border rounded-2xl p-6 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 bg-purple-100 dark:bg-purple-900/30 rounded-xl flex items-center justify-center text-2xl">⚡</div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-foreground">Custom Ad Provider</h4>
                        <p className="text-sm text-muted-foreground">Add any ad network using custom HTML/JS embed code</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Provider Name</label>
                        <input type="text" placeholder="e.g., Taboola, Outbrain" className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">Embed Code</label>
                        <textarea placeholder="Paste your ad provider's embed code..." rows={3} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm font-mono resize-none" />
                      </div>
                ...
                <div className="border border-border rounded-2xl p-6">
                  <h4 className="font-semibold text-foreground mb-4">Global Ad Settings</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <span className="text-sm text-foreground">Enable ads for logged-in users</span>
                      <input type="checkbox" className="rounded" defaultChecked />
                    </label>
                    <label className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <span className="text-sm text-foreground">Show ads on mobile</span>
                      <input type="checkbox" className="rounded" defaultChecked />
                    </label>
                    <label className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <span className="text-sm text-foreground">Lazy-load advertisements</span>
                      <input type="checkbox" className="rounded" defaultChecked />
                    </label>
                    <label className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <span className="text-sm text-foreground">Ad-free for premium users</span>
                      <input type="checkbox" className="rounded" />
                    </label>
                  </div>
                </div>
