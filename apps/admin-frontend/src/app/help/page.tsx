...
export default function HelpPage() {
 ...
 <h1 className="text-3xl sm:text-4xl font-bold text-[rgb(var(--foreground))]">Help Center</h1>
 <p className="text-[rgb(var(--muted-foreground))] mt-2 max-w-xl mx-auto">
 Guides, FAQs, and keyboard shortcuts for the NewsTRNT Admin Panel
 </p>
 <div className="relative max-w-lg mx-auto mt-6">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[rgb(var(--muted-foreground))]">🔍</span>
 <input
 type="text"
 placeholder="Search help articles..."
 value={searchTerm}
 onChange={(e) => setSearchTerm(e.target.value)}
 className="w-full pl-11 pr-4 py-3 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl text-[rgb(var(--foreground))] placeholder:text-[rgb(var(--muted-foreground))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))]/40 shadow-sm"
 />
 </div>
 </div>
 ...
 key={link.href}
 href={link.href}
 className="flex flex-col items-center gap-2 p-4 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl hover:bg-[rgb(var(--muted))]/5 hover:border-[rgb(var(--primary))]/20 transition-all group"
 >
 <span className="text-2xl group-hover:scale-110 transition-transform">{link.icon}</span>
 <span className="text-xs font-medium text-[rgb(var(--muted-foreground))] group-hover:text-[rgb(var(--foreground))]">{link.label}</span>
 </Link>
 ))}
 </div>
 ...
 <div className="bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl p-5 sticky top-6 space-y-6">
 {/* Category Filter */}
 <div>
 <h3 className="text-sm font-semibold text-[rgb(var(--foreground))] mb-3">Browse Topics</h3>
 ...
 :'text-[rgb(var(--muted-foreground))] hover:bg-[rgb(var(--muted))]/5 hover:text-[rgb(var(--foreground))]'
 }`}
 >
 ...
 {/* Keyboard Shortcuts */}
 <div>
 <h3 className="text-sm font-semibold text-[rgb(var(--foreground))] mb-3">Keyboard Shortcuts</h3>
 <div className="space-y-2">
 {SHORTCUTS.map(s => (
 <div key={s.keys} className="flex items-center justify-between text-xs">
 <kbd className="px-2 py-0.5 bg-[rgb(var(--muted))]/10 border border-[rgb(var(--border))] rounded font-mono text-[rgb(var(--muted-foreground))]">{s.keys}</kbd>
 <span className="text-[rgb(var(--muted-foreground))]">{s.action}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <h2 className="text-lg font-semibold text-[rgb(var(--foreground))]">User Guides</h2>
 <p className="text-sm text-[rgb(var(--muted-foreground))] mt-0.5">Step-by-step guides for every feature</p>
 </div>
 <div className="p-6 space-y-8">
 {GUIDES.map(section => (
 <div key={section.section}>
 <h3 className="flex items-center gap-2 text-sm font-semibold text-[rgb(var(--foreground))] mb-3">
 ...
 key={guide.link}
 href={guide.link}
 className="flex items-start gap-3 p-4 rounded-xl border border-[rgb(var(--border))] hover:bg-[rgb(var(--muted))]/10 hover:border-[rgb(var(--primary))]/20 transition-all group"
 >
 <span className="text-xl mt-0.5">{guide.icon}</span>
 <div className="flex-1 min-w-0">
 <p className="font-medium text-[rgb(var(--foreground))] text-sm group-hover:text-[rgb(var(--primary))] transition-colors">{guide.title}</p>
 <p className="text-xs text-[rgb(var(--muted-foreground))] mt-0.5 line-clamp-2">{guide.description}</p>
 <span className="text-[11px] text-[rgb(var(--muted-foreground))] mt-1 inline-block">{guide.readTime} read</span>
 </div>
 </Link>
 ))}
 </div>
 ...
 <p className="text-sm text-[rgb(var(--muted-foreground))] mt-0.5">{filteredFAQs.length} questions</p>
 </div>
 ...
 className="w-full text-left px-6 py-4 flex items-center justify-between hover:bg-[rgb(var(--muted))]/10/30 transition-colors"
 >
 <span className="font-medium text-[rgb(var(--foreground))] text-sm pr-4">{faq.question}</span>
 <span className={`text-[rgb(var(--muted-foreground))] shrink-0 transition-transform duration-200 ${expandedFAQ === faq.id ?'rotate-180' :''}`}>▾</span>
 </button>
 {expandedFAQ === faq.id && (
 <div className="px-6 pb-5 text-sm text-[rgb(var(--muted-foreground))] leading-relaxed animate-in fade-in slide-in-from-top-1 duration-200">
 {faq.answer}
 </div>
 )}
 </div>
 ...
 {/* Contact Support */}
 <div className="bg-gradient-to-br from-blue-500/5 to-purple-500/5 border border-[rgb(var(--border))] rounded-xl p-8 text-center">
 <h2 className="text-lg font-semibold text-[rgb(var(--foreground))] mb-2">Still need help?</h2>
 <p className="text-sm text-[rgb(var(--muted-foreground))] mb-5 max-w-md mx-auto">
 Contact the development team or check the debug tools for system diagnostics.
 </p>
 <div className="flex flex-wrap items-center justify-center gap-3">
 <a
 href="mailto:support@newstrnt.com"
 className="px-5 py-2.5 bg-[rgb(var(--primary))] text-white rounded-xl text-sm font-medium hover:opacity-90 transition-colors"
 >
 Email Support
 </a>
 <Link
 href="/dev-tools/debug"
 className="px-5 py-2.5 bg-[rgb(var(--card))] border border-[rgb(var(--border))] text-[rgb(var(--foreground))] rounded-xl text-sm font-medium hover:bg-[rgb(var(--muted))]/5 transition-colors"
 >
 System Diagnostics
 </Link>
 <Link
