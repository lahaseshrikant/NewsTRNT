...
export default function SourcesPage() {
 ...
 <p className="text-[rgb(var(--muted-foreground))]/70 text-sm mt-1">News feeds, APIs, and market data sources</p>
 </div>
 ...
 {/* Source Groups */}
 {groups.map((group) => (
 <div key={group.type} className="bg-[#1a1a2e] rounded-xl border border-[rgb(var(--border))]/50 p-5">
 <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-4">
 {group.icon} {group.label}
 <span className="text-xs text-[rgb(var(--muted-foreground))] font-normal">({group.sources.length})</span>
 </h2>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
 {group.sources.length === 0 ? (
 <p className="text-[rgb(var(--muted-foreground))] text-sm col-span-2">No sources configured</p>
 ) : (
 ...
 >
 <span className={`w-2 h-2 rounded-full ${src.enabled !== false ?'bg-green-400' :'bg-[rgb(var(--muted-foreground))]'}`} />
 <div className="flex-1 min-w-0">
 <div className="text-white text-sm font-medium truncate">{src.name}</div>
 {src.url && (
 <div className="text-xs text-[rgb(var(--muted-foreground))] truncate">{src.url}</div>
 )}
 {src.category && (
 <span className="text-xs text-[rgb(var(--muted-foreground))]/70 bg-[rgb(var(--card))] px-1.5 py-0.5 rounded mt-0.5 inline-block">
 {src.category}
 </span>
 )}
 </div>
 <span className={`text-xs ${src.enabled !== false ?'text-green-400' :'text-[rgb(var(--muted-foreground))]'}`}>
