...
export default function ContentEngineDashboard() {
 ...
 default: return'text-[rgb(var(--muted-foreground))]/70';
 }
 };
 ...
 case'running': case'in_progress': return'bg-[rgb(var(--primary))]/50/10 border-[rgb(var(--primary))]/20';
 ...
 default: return'bg-[rgb(var(--muted-foreground))]/80/10 border-[rgb(var(--border))]/20';
 }
 };
 ...
 <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[rgb(var(--primary))] mx-auto mb-4" />
 <p className="text-[rgb(var(--muted-foreground))]/70">Loading Content Engine...</p>
 </div>
 </div>
 );
 }
 ...
 <p className="text-[rgb(var(--muted-foreground))]/70 mt-1">
 ...
 triggerLoading === type
 ?'bg-[rgb(var(--primary))]/50/10 border-[rgb(var(--primary))]/30 animate-pulse'
 :'bg-[#1a1a2e] border-[rgb(var(--border))]/50 hover:border-[rgb(var(--primary))]/40 hover:bg-[rgb(var(--primary))]/50/5'
 }`}
 >
 <div className="flex items-center justify-between">
 <div>
 <div className="text-sm text-[rgb(var(--muted-foreground))]/70 uppercase tracking-wider mb-1">
 ...
 <div className="bg-[#1a1a2e] rounded-xl border border-[rgb(var(--border))]/50 p-5">
 ...
 <span className="text-[rgb(var(--muted-foreground))]/50 text-sm font-medium">{run.type}</span>
 </div>
 <span className="text-[rgb(var(--muted-foreground))] text-xs">{formatTime(run.started_at)}</span>
 </div>
 ...
 <div
 key={job.id}
 className="p-3 rounded-lg bg-[rgb(var(--muted))]/15 border border-[rgb(var(--border))]/30"
 >
 <div className="flex items-center justify-between">
 <div>
 <span className="text-white text-sm font-medium">{job.name || job.id}</span>
 <span className="text-[rgb(var(--muted-foreground))] text-xs ml-2">({job.trigger})</span>
 </div>
 ...
 <thead>
 <tr className="text-left text-[rgb(var(--muted-foreground))]/70 border-b border-[rgb(var(--border))]/50">
 ...
 <tr>
 <td colSpan={7} className="text-center text-[rgb(var(--muted-foreground))] py-6">
 ...
 : null;
 return (
 <tr key={run.id} className="border-b border-[rgb(var(--border))]/50">
 <td className="py-2 pr-4">
 <span className={`text-xs font-mono ${statusColor(run.status)}`}>
 {run.status}
 </span>
 </td>
 <td className="py-2 pr-4 text-[rgb(var(--muted-foreground))]/50">{run.dataType}</td>
 <td className="py-2 pr-4 text-[rgb(var(--muted-foreground))]/50">{run.itemsFound ??'—'}</td>
 <td className="py-2 pr-4 text-green-400">{run.itemsInserted ??'—'}</td>
 <td className="py-2 pr-4 text-red-400">{run.itemsFailed ||'—'}</td>
 <td className="py-2 pr-4 text-[rgb(var(--muted-foreground))]/70">{formatTime(run.startedAt)}</td>
 <td className="py-2 text-[rgb(var(--muted-foreground))]/70">{duration != null ? `${duration}s` :'—'}</td>
 </tr>
 );
 })
 )}
 ...
 <div className="p-3 rounded-lg bg-[rgb(var(--muted))]/15 border border-[rgb(var(--border))]/30">
 <div className="text-blue-400 font-medium mb-1">Scraping Layer</div>
 <ul className="text-[rgb(var(--muted-foreground))]/70 space-y-1 text-xs">
...
function StatCard({
 ...
 return (
 <div className="bg-[#1a1a2e] rounded-xl border border-[rgb(var(--border))]/50 p-4">
 <div className="flex items-center gap-2 text-[rgb(var(--muted-foreground))]/70 text-sm mb-1">
 <span>{icon}</span>
 <span>{label}</span>
 </div>
 <div className={`text-2xl font-bold ${colorMap[color]}`}>{value}</div>
 {sub && <div className="text-xs text-[rgb(var(--muted-foreground))] mt-1 truncate">{sub}</div>}
 </div>
 );
}
