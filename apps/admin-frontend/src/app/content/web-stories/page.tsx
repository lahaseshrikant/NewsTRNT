...
export default function WebStoriesAdmin() {
 ...
 archived:'bg-[rgb(var(--muted))]/10 text-[rgb(var(--muted-foreground))]',
 };
 ...
 <div>
 <h1 className="text-2xl sm:text-2xl font-semibold tracking-tight text-[rgb(var(--foreground))]">Web Stories</h1>
 <p className="text-[rgb(var(--muted-foreground))] mt-1">Create immersive visual stories for mobile-first audiences</p>
 </div>
 <div className="flex gap-3">
 <Link
 href="/content/web-stories/create"
 className="px-5 py-2.5 bg-[rgb(var(--primary))] text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-colors shadow-sm"
 >
 + Create Story
 </Link>
 <button className="px-4 py-2.5 bg-[rgb(var(--card))] border border-[rgb(var(--border))] text-[rgb(var(--foreground))] rounded-xl text-sm font-medium hover:bg-[rgb(var(--muted))]/5 transition-colors">
 Import
 </button>
 </div>
 </div>
 ...
 <div key={stat.label} className={`bg-gradient-to-br ${stat.color} border border-[rgb(var(--border))] rounded-xl p-5`}>
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs font-medium text-[rgb(var(--muted-foreground))] uppercase tracking-wide">{stat.label}</p>
 ...
 {/* Toolbar */}
 <div className="bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl p-4 mb-6">
 <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
 <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
 <div className="relative flex-1 min-w-[200px] lg:min-w-[280px]">
 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[rgb(var(--muted-foreground))] text-sm">🔍</span>
 <input
 type="text"
 placeholder="Search stories..."
 value={searchTerm}
 onChange={(e) => setSearchTerm(e.target.value)}
 className="w-full pl-9 pr-4 py-2 bg-[rgb(var(--background))] border border-[rgb(var(--border))] rounded-lg text-sm text-[rgb(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))]/40"
 />
 </div>
 <select
 value={filterStatus}
 ...
 <button onClick={() => handleBulkAction('publish')} className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-medium hover:bg-emerald-700">
 ...
 <div className="flex bg-[rgb(var(--muted))]/10 rounded-lg p-0.5">
 <button onClick={() => setViewMode('grid')} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${viewMode ==='grid' ?'bg-[rgb(var(--card))] text-[rgb(var(--foreground))] shadow-sm' :'text-[rgb(var(--muted-foreground))]'}`}>
 Grid
 </button>
 <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${viewMode ==='list' ?'bg-[rgb(var(--card))] text-[rgb(var(--foreground))] shadow-sm' :'text-[rgb(var(--muted-foreground))]'}`}>
 List
 </button>
 </div>
 </div>
 ...
 <thead className="bg-[rgb(var(--muted))]/5 border-b border-[rgb(var(--border))]">
 <tr>
 ...
 <th className="p-4 text-left text-xs font-semibold text-[rgb(var(--muted-foreground))] uppercase tracking-wider">Story</th>
 <th className="p-4 text-left text-xs font-semibold text-[rgb(var(--muted-foreground))] uppercase tracking-wider hidden sm:table-cell">Status</th>
 <th className="p-4 text-left text-xs font-semibold text-[rgb(var(--muted-foreground))] uppercase tracking-wider hidden md:table-cell">Performance</th>
 <th className="p-4 text-left text-xs font-semibold text-[rgb(var(--muted-foreground))] uppercase tracking-wider hidden lg:table-cell">Date</th>
 <th className="p-4 text-right text-xs font-semibold text-[rgb(var(--muted-foreground))] uppercase tracking-wider">Actions</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border">
 {filteredStories.map(story => (
 <tr key={story.id} className="hover:bg-[rgb(var(--muted))]/10/30 transition-colors">
 <td className="p-4">
 <input type="checkbox" checked={selectedStories.includes(story.id)} onChange={() => handleSelectStory(story.id)} className="rounded border-[rgb(var(--border))]" />
 </td>
 <td className="p-4">
 ...
 <Link href={`/content/web-stories/create?id=${story.id}`} className="font-medium text-[rgb(var(--foreground))] text-sm hover:text-[rgb(var(--primary))] line-clamp-1">
 {story.title}
 </Link>
 ...
 <div className="text-xs space-y-0.5 text-[rgb(var(--muted-foreground))]">
 <span className="flex items-center gap-1">{fmtViews(story.views)}</span>
 <span className="flex items-center gap-1">{story.likes} {story.shares}</span>
 </div>
 </td>
 <td className="p-4 hidden lg:table-cell text-xs text-[rgb(var(--muted-foreground))]">{new Date(story.publishedAt).toLocaleDateString()}</td>
 <td className="p-4 text-right">
 <div className="flex items-center justify-end gap-2">
 <Link href={`/content/web-stories/create?id=${story.id}`} className="text-[rgb(var(--primary))] hover:text-[rgb(var(--primary))] text-xs font-medium">
 Edit
 </Link>
 ...
 <h3 className="text-xl font-semibold text-[rgb(var(--foreground))] mb-2">No web stories found</h3>
 ...
 <Link
 href="/content/web-stories/create"
 className="inline-flex px-6 py-3 bg-[rgb(var(--primary))] text-white rounded-xl font-medium hover:opacity-90 transition-colors"
 >
 Create First Story
 </Link>
 </div>
 ...
 {/* Pagination */}
 {!loading && filteredStories.length > 0 && (
 <div className="flex items-center justify-between mt-6 text-sm text-[rgb(var(--muted-foreground))]">
 <span>Showing {filteredStories.length} of {totalStories} stories</span>
 <div className="flex gap-1">
 <button className="px-3 py-1.5 border border-[rgb(var(--border))] rounded-lg hover:bg-[rgb(var(--muted))]/5 transition-colors">Prev</button>
 <button className="px-3 py-1.5 bg-[rgb(var(--primary))] text-white rounded-lg font-medium">1</button>
 <button className="px-3 py-1.5 border border-[rgb(var(--border))] rounded-lg hover:bg-[rgb(var(--muted))]/5 transition-colors">Next</button>
 </div>
 </div>
 )}
 </div>
